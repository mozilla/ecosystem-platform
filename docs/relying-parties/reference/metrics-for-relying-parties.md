...
Fivetran preserves historical data within `subscription_history` by comparing the current state of the subscription with the most recent record in the history table and adding a new record if they vary.
...
Subscription Platform handles getting Apple subscriptions from the Apple API and stores the latest state of active subscriptions in Firestore. Prior to December 2022, VPN Apple subscriptions data was managed by VPN's Guardian service and stored in the Guardian database. As Firestore doesn't contain the full historical state of Apple subscriptions, Guardian data is still used for historical purposes.
