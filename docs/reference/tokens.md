...
A **_Bearer Token_** is an opaque string that can be used by any client in posession of the token, simply by presenting
...
A **_Hawk Token_** consists of an opaque string id and corresponding secret key. To use the token, clients must use the
...
An **_FxA Hawk Token_** is a 32-byte string from which the id and key of a **_Hawk Token_** can be generated using HKDF.
This simplifies client handling of the token since it only has to persist a single value.

A **_Prefixed Bearer Token_** is an FxA token sent as `Authorization: Bearer <prefix>_<hex>`, where the prefix marks the token kind (for example `fxs_` for a session token). This is the preferred scheme for every client, and the one new integrations should use. In-monorepo callers ([`fxa-auth-client`](https://github.com/mozilla/fxa/tree/main/packages/fxa-auth-client) to auth-server) already use it. Hawk remains only as a fallback for the clients still on it. See [Authentication Schemes](authentication-schemes) for the full prefix table.
...
Session tokens are a 32-byte string generated by the server. The preferred way to send them is as a prefixed Bearer token (`Bearer fxs_<hex>`); Firefox Desktop, iOS, and Android still use Hawk today and are planned to move to Bearer (see [Authentication Schemes](authentication-schemes)). They can be obtained by:
...
- `tokenVerified` checks for the presence of `tokenVerificationId`, i.e. whether an entry exists in the `unverifiedTokens` table.
- `mustVerify` on the session token was historically a flag used only when the request requested scoped keys. More recently, it is also set when `tokenVerificationId` is set.

Types of unverified sessions:

1. **Scoped keys were requested** (e.g. Sync sign-in) and the user hasn't entered the emailed OTP code.
2. **The user has 2FA** and hasn't entered their TOTP code.
3. **Server-side heuristics** — based on factors such as whether the account is newly-created or whether we've seen previous verified logins from the same IP address. Logins from RP logins/redirects are allowed to skip entering the code for less friction. The next time that user tries to perform an action in account settings, they'll be required to verify.

:::note For FxA engineers
To get into the third type (heuristic-based unverified session) locally, use a `@mozilla.com` email address. The `SIGNIN_CONFIRMATION_FORCE_EMAIL_REGEX` env var controls which email addresses are forced into sign-in confirmation with `mustVerify: 0`. To force sign-in confirmation globally (with `mustVerify: 1`, simulating a suspicious request or scoped key request), set `SIGNIN_CONFIRMATION_FORCE_GLOBALLY=true`. See the auth-server `config/index.ts` under `signinConfirmation` for more details.
:::
...
This is a long-lived **_Bearer Token_** that represents the permissions granted by the user to that RP.
...
**_Bearer Token_** that represents the permissions granted by the user to that RP, in a format that can
...
They're for fetching keys. RPs shouldn't need to know this, because they use the scoped-keys flow. Sent as `Bearer fxk_<hex>`.

### Password Change Tokens

A special-use token for changing password, sent as `Bearer fxpc_<hex>`. Hawk is a legacy fallback for clients that have not moved to Bearer yet.

### Password Forgot Tokens

This is a special-use token for resetting a password. The user needs to be verified via a code received over email. This has a limited number of verification attempts and generates a reset token. Sent as `Bearer fxpf_<hex>`.
...
- User completes an email confirmation loop to confirm password reset.
  - This produces an `accountResetToken`, a credential for subsequent requests (sent as `Bearer fxar_<hex>`).
